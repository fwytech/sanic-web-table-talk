...
class MinioUtils:
    ...
    def upload_to_minio_form_stream(
        ...
        try:
            ...
            if self.enabled and self.client:
                ...
                file_path = os.path.join(self.local_base, bucket_name, file_name)
                with open(file_path, "wb") as f:
                    f.write(file_stream.getvalue())
            return file_name
    ...
    def upload_file_and_parse_from_request(self, request: Request, bucket_name: str = "filedata") -> dict:
        ...
        try:
            ...
            # 创建一个txt文件并上传
            parse_file_key = self.upload_to_minio_form_stream(io.BytesIO(full_text.encode("utf-8")), bucket_name, object_name + file_suffix)
            return {
        ...
        for file_info in file_info_list:
            ...
            try:
                if self.enabled and self.client:
                    ...
                    file_path = os.path.join(self.local_base, bucket_name, parse_file_key)
