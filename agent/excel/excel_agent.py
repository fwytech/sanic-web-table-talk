...
class ExcelAgent:
    ...
    async def run_excel_agent(
        ...
        try:
            ...
            await response.write("data:" + json.dumps({"data": "DONE", "dataType": DataTypeEnum.STREAM_END.value[0]}, ensure_ascii=False) + "\n\n")
    ...
    async def _process_step_content(
        ...
        if step_name in content_map:
            ...
            if step_name == "data_render" and content:
                content = "\n---\n" + content

            # 适配EChart表格
            # 根据内容与步骤动态选择数据类型，支持AntV失败回退ECharts
            if step_name == "data_render_apache":
                data_type = DataTypeEnum.BUS_DATA.value[0]
            elif step_name == "data_render" and (not content) and step_value.get("apache_chart_data"):
                data_type = DataTypeEnum.BUS_DATA.value[0]
                content = step_value.get("apache_chart_data", {})
            else:
