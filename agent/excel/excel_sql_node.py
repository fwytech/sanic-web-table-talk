...
def sql_generate_excel(state):
    ...
    try:
        ...
        sql_query = str(result_obj.get("sql_query", "")).strip()
        chart_type = str(result_obj.get("chart_type", "generate_table")).strip()
        ...
        if sql_query.upper() == "NULL" or len(sql_query) == 0:
            ...
            sql_query = f"SELECT {', '.join(select_cols)} FROM {table_name} LIMIT 50" if len(select_cols) > 0 else f"SELECT * FROM {table_name} LIMIT 50"
            chart_type = "generate_table"

        state["generated_sql"] = sql_query
        state["chart_type"] = "mcp-server-chart-" + chart_type
