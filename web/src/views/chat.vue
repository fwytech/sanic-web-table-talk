  ...
  >
    <n-layout has-sider class="custom-layout">
      <n-layout-sider
        ...
        :collapsed-width="70"
        :width="210"
        collapsed-style="min-height: 100%"
        trigger-style="top: 44%; left: 19px; height: calc(10px + 3em); border-radius: 28px;"
        :collapsed="collapsed"
        :auto-collapse="false"
        theme-overrides="{
          sider:
      ...
      >
        <div class="custom-sider">
          <template v-if="!showDefaultPage">
            <div class="header">
              <n-space justify="space-between">
                <div class="create-chat-box" :class="{ hide: isFocusSearchChat }">
                  <transition name="fade">
                    <template v-if="!isFocusSearchChat">
                      <n-button
                        type="default"
                        ghost
                        :dashed="true"
                ...
                <div
                  ml-1
                  class="search-chat"
                  :class="{ focus: isFocusSearchChat }"
                >
                  <div flex="~ justify-between gap-10 items-center" class="w-full" p-3>
                    <div class="ml-l text-4 cursor-pointer">
                      <div class="i-ep:search color" @click="onFocusSearchChat"></div>
                    </div>
                    <div class="flex-1">
                      <n-input
                        ref="searchChatRef"
                    ...
                    <div class="mr-1">
                      <div class="icon-button" :class="[isFocusSearchChat ? 'selected' : '']">
                        <div class="i-mingcute:search-3-line" @click="onFocusSearchChat"></div>
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      return h(
                        'div',
                        {
                          style: {
                            ...
                            'box-shadow': row.qa_type === qa_type ? '4px 4px 20px 4px rgba(0,0,0,0.04)' : 'none',
                            'z-index': row.qa_type === qa_type ? '1' : '0',
                        ...
                        [
                          ...
                          }, [h('div', { style: { 'font-size': '12px', color: `#7f818e` } }, `${dayjs().format('YYYY年MM月DD日 HH:mm')}`)]),
                        ],
                      )
                    },
                  },
      ...
      <n-layout-content>
        ...
        >
          <NavigationNavBar>
            <!-- 渲染CHAT header -->
            <template #bottom>
              <div flex="~ items-center justify-between" px-36px>
                <div flex="~ items-center gap-12" class="text-12" select-none>
                  <div class="rounded-10px p-5 hover:bg-#ffffff/20 transition-all-300">
                    <div class="size-20 i-majesticons:chat-line"></div>
                  </div>
                  <div>Chat</div>
                </div>
                ...
                <div flex="~ gap-10">
                  <div
                    class="icon-button"
                    :class="[ qa_type === 'COMMON_QA' ? 'selected' : '']"
                    @click="onAqtiveChange('COMMON_QA', '')"
                  >
                    <div class="i-google-symbols:assistant"></div>
                  </div>
                  <div
                    class="icon-button"
                    :class="[ qa_type === 'FILEDATA_QA' ? 'selected' : '']"
                    @click="onAqtiveChange('FILEDATA_QA', '')"
                  >
                    <div class="i-majesticons:document-line"></div>
                  </div>
                  <div
                    class="icon-button"
                    :class="[ qa_type === 'DATABASE_QA' ? 'selected' : '']"
                    @click="onAqtiveChange('DATABASE_QA', '')"
                  >
                    <div class="i-majesticons:database-line"></div>
                  </div>
                  <div
                    class="icon-button"
                    :class="[ qa_type ===  'REPORT_QA' ? 'selected' : '']"
                    @click="onAqtiveChange('REPORT_QA', '')"
                  >
                    <div class="i-ic:twotone-insights"></div>
                  </div>
                </div>
              </div>
            </template>
          ...
          <div style="background-color: #f6f7fb">
            <div flex="1 ~" min-w-0 h-full overflow-hidden p-10 px-36px>
              <n-space v-if="showDefaultPage === false" vertical :size="24">
                <div w-full>
                      ...
                      <template #icon>
                        <n-icon size="18">
                          ...
                          >
                            ...
                            />
                            <path
                              d="M137 66l37 63h-74zM921 921l-63 37v-74zM287 381h66c17.1 0 31 13.9 31 31v354c0 17.1-13.9 31-31 31h-66c-17.1 0-31-13.9-31-31V412c0-17.1 13.9-31 31-31zM491 193h66c17.1 0 31 13.9 31 31v542c0 17.1-13.9 31-31 31h-66c17.1 0-31-13.9-31-31V224c0-17.1 13.9-31 31-31zM695 469h66c17.1 0 31 13.9 31 31v266c0 17.1-13.9 31-31 31h-66c-17.1 0-31-13.9-31-31V500c0-17.1 13.9-31 31-31z"
                              fill="#222222"
                              p-id="25830"
                            />
                          </svg>
                ...
                >
                  ...
                  <n-input
                    ...
                    type="textarea"
                    class="textarea-resize-none w-full text-15 [&_.n-input__border]:hidden [&_.n-input__state-border]:hidden [&_.n-input-wrapper]:p-0!"
                    :style="{
